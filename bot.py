...
invalid_smtp = set()        # emails de comptes qui ont échoué récemment (compat)

# ================= POOL GMAIL API =================
gmail_pool = GmailAPIPool(SMTP_ACCOUNTS)
print(f"📨 Gmail API : {len(gmail_pool.accounts)} compte(s) prêt(s) sur {len(SMTP_ACCOUNTS)}")

def rebuild_pool():
    """Recharge le pool quand SMTP_ACCOUNTS est modifié à chaud."""
    global gmail_pool
    gmail_pool = GmailAPIPool(SMTP_ACCOUNTS)

def test_smtp_connection():
    """Teste tous les comptes via l'API Gmail (HTTPS)."""
...
def send_email(account, to, subject, body, sender_name):
    """Envoie un email via l'API Gmail. Signature compatible existant."""
    ok, err = gmail_pool.send_email(account, to, subject, body, sender_name)
    # Synchronise le set legacy
...
def add_smtp_account_json(chat_id, raw_json):
    """Ajoute un compte Gmail OAuth depuis un blob JSON envoyé par l'admin."""
...
# Alias pour compat (ancienne signature email/password)
...
def admin_test_smtp(chat_id):
    """Commande /test_smtp : teste tous les comptes via l'API Gmail."""
    if not gmail_pool.accounts:
...
def admin_stats(chat_id):
    msg = (f"📊 Statistiques:\n\n"
           ...
           f"Transport: Gmail API HTTPS (port 443)\n"
...
def main():
    ...
    print(f"   Transport: Gmail API (HTTPS, port 443)")
    print(f"   Comptes Gmail chargés: {len(SMTP_ACCOUNTS)}")
